...
/// Factory for creating reducer worker pools in the thread-socket implementation
pub struct SocketReducerFactory<P, S, R, SD>
where
    P: MapReduceProblem,
    S: StateAccess,
    R: WorkerRuntime,
...
impl<P, S, R, SD> SocketReducerFactory<P, S, R, SD>
where
    P: MapReduceProblem,
    S: StateAccess,
    R: WorkerRuntime,
...
where
    ...
    P::ReduceAssignment: serde::Serialize + for<'de> serde::Deserialize<'de>,
    S: StateAccess,
    ...
    SD: ShutdownSignal + Sync,
{
    ...
    pub fn create_pool_and_distributor(
        ...
        Vec<Reducer<P, S, SocketWorkChannel<P::ReduceAssignment, CompletionSender>, R, SD>>,
        SocketWorkDistributor<
            Reducer<P, S, SocketWorkChannel<P::ReduceAssignment, CompletionSender>, R, SD>,
            Box<
                dyn FnMut(
                        usize,
                    ) -> Reducer<
                        P,
                        S,
                        SocketWorkChannel<P::ReduceAssignment, CompletionSender>,
                        R,
                        SD,
                    > + Send,
            >,
...
impl<P, S, R, SD> WorkerPoolFactory for SocketReducerFactory<P, S, R, SD>
where
    P: MapReduceProblem + 'static + Sync,
    P::ReduceAssignment: serde::Serialize + for<'de> serde::Deserialize<'de>,
    S: StateAccess,
    ...
    SD: ShutdownSignal + Sync,
{
    type Worker = Reducer<P, S, SocketWorkChannel<P::ReduceAssignment, CompletionSender>, R, SD>;
