...
/// Factory for creating mapper worker pools in the task-channels implementation
pub struct TaskMapperFactory<P, S, R, SD>
where
    P: MapReduceProblem,
    S: StateAccess,
    R: WorkerRuntime,
...
impl<P, S, R, SD> TaskMapperFactory<P, S, R, SD>
where
    P: MapReduceProblem,
    S: StateAccess,
    R: WorkerRuntime,
    ...
    SD: ShutdownSignal + Sync,
{
    ...
    pub fn create_pool_and_distributor(
        ...
        Vec<Mapper<P, S, MpscWorkChannel<P::MapAssignment, Sender<CompletionMessage>>, R, SD>>,
        TaskWorkDistributor<
            Mapper<P, S, MpscWorkChannel<P::MapAssignment, Sender<CompletionMessage>>, R, SD>,
            ...
            Box<
                dyn FnMut(
                        ...
                        P,
                        S,
                        MpscWorkChannel<P::MapAssignment, Sender<CompletionMessage>>,
                        R,
                        SD,
                    > + Send,
            >,
...
impl<P, S, R, SD> WorkerPoolFactory for TaskMapperFactory<P, S, R, SD>
where
    P: MapReduceProblem + 'static + Sync,
    S: StateAccess,
    R: WorkerRuntime + Sync,
    SD: ShutdownSignal + Sync,
{
    type Worker = Mapper<P, S, MpscWorkChannel<P::MapAssignment, Sender<CompletionMessage>>, R, SD>;
